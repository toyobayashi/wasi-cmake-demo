...
if (typeof __webpack_public_path__ !== 'undefined') {
  ...
  const wasmModule = await WasmModule.load(wasmUrl, imports, wasi, true)
  ...
  const wasi = new WASI(wasiOptions)
  const wasmModule = await WasmModule.load(isNodeJs ? await (await import('node:fs/promises')).readFile(url) : url, imports, wasi, true)
  wasm = wasmModule.instance.exports
  await (wasmModule.run().ret)
}
